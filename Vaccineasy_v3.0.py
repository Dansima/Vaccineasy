...
# --- CONFIGURARE PAGINĂ ---
st.set_page_config(page_title="Cabinet Vaccinări V3.1", layout="wide")
...
def decode_cnp(cnp):
    ...
    cnp = str(cnp).strip()
    # Curățăm CNP-ul de caractere ciudate
...
def get_status_vaccinare(data_nasterii):
    ...
    # FILTRU ADULȚI: Ignorăm ce e peste 15 ani
    if varsta_ani > 15:
...
# --- 2. LOGICA DE IMPORT (CSV + EXCEL) ---
def incarca_date_din_excel(uploaded_file):
    """Citeste Excel SAU CSV si normalizeaza coloanele."""
    try:
        ...
        if uploaded_file.name.endswith('.csv'):
            try:
                ...
                st.error("Nu am putut citi formatul CSV. Verifică separatorul.")
                return pd.DataFrame()
        else:
            # E Excel
        ...
        if not col_nume or not col_cnp:
            st.error(f"Nu am găsit coloanele 'Nume' și 'CNP'. Coloane găsite: {list(df.columns)}")
            return pd.DataFrame()
        ...
        for index, row in df.iterrows():
            ...
            nume_complet = str(row[col_nume]).strip()

            data_nasterii = decode_cnp(cnp)
            status, vaccin, cod_cat = get_status_vaccinare(data_nasterii)

            if "Adult" in status or "Eroare" in status:
                continue

            processed_data.append({
                "Nume si Prenume": nume_complet,
                "CNP": cnp,
...
# --- 3. GENERATOR EXCEL (DESIGN ANEXA 1) ---
def convert_df_to_catagrafie(df_input):
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        ...
        worksheet.merge_range('A5:Y5', f'în luna {datetime.now().month} / anul {datetime.now().year}', fmt_title)

        # Configurare Coloane
        worksheet.set_column(0, 0, 4)  # Nr Crt
        worksheet.set_column(1, 1, 30)  # Nume
        worksheet.set_column(2, 2, 15)  # CNP
        worksheet.set_column(3, 24, 3.5)  # Vaccinuri
        ...
        current_row += 2
        worksheet.write(current_row, 0, 'NOTĂ: Generat automat din aplicație.', fmt_top_left)
...
st.title("💉 Sistem Vaccinări - V3.1 (Producție)")
    ...
    if not df.empty:
        ...
        with col_btn:
            ...
            st.download_button(
                "📥 Descarcă Anexa 1",
                data=excel_data,
                file_name=f'Catagrafie_{datetime.now().strftime("%B_%Y")}.xlsx',
